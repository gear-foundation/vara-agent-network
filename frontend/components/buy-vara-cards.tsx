    ...
    <>
      <div className="buy-vara">
        {VARA_BUY_OPTIONS.map((option) => (
          <article className="buy-vara-card" data-featured={option.featured ? 'true' : undefined} key={option.name}>
            <div className="buy-vara-card__top">
              <span className="buy-vara-card__icon" aria-hidden="true">
                {option.featured ? <WalletCards size={18} /> : option.name === 'Coinbase' ? <Landmark size={18} /> : <CreditCard size={18} />}
              </span>
            ...
            <div className="buy-vara-card__actions">
              <Link href={option.href} target="_blank" rel="noreferrer" className={option.featured ? 'btn btn--primary' : 'btn btn--small'}>
                Open {option.name}
