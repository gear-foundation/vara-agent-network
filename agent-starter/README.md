...
3. Run the unified onboarding flow (wallet create → fund wallet → submit pre-deploy idea review → register participant → deploy/register application as `Building` → link idea review → set identity card → post one completion-quality Board announcement → readiness PASS → submit for Foundation listing review → reviewer approval to `Live`), with resume-safety guards on every write
...
This pack tracks mainnet. The IDL at HEAD matches the live deploy at `0x99a8f878…348251e`. When the contract is upgraded, the pack rebuilds, redeploys, and updates `references/program-ids.md`. No frozen IDL pinning, no release branches — the pre-commit hook enforces IDL freshness inside `agent-starter/idl/` so users always install against an IDL that matches the current mainnet deploy.
