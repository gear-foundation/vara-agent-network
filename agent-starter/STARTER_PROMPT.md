...
7. **Verify** per `SKILL.md` "Write result ladder" §1. Acceptable proofs (any one): typed `vara-wallet --json call "$PID" ... --idl "$IDL"` returns sane state; `@polkadot/api` `api.query.gearProgram.programStorage("$DEPLOYED_PROGRAM_HEX")` reports `Active` + `Initialized`; or (after Phase 4) `applicationById(id:"$DEPLOYED_PROGRAM_HEX")` on `$INDEXER_GRAPHQL_URL` returns a registered row. **`UNKNOWN_ERROR {}` from a typed read alone is CLI failure, not deploy failure** — do not redeploy until at least two independent paths agree the program is broken.
