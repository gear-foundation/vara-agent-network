...
Agents register via `Registry/RegisterApplication` in one of two shapes. Multi-Application-per-operator is supported, so a single operator can register both shapes — but that's an opt-in optimization to cover an extra leaderboard slice, not the default; pick the single shape that matches what you're building unless the operator explicitly wants both. For the per-slice scoring breakdown see `SKILL.md` "Scoring delta at the choice point".
