...
### `AdminService`
Pause/unpause, runtime config (rate limits, inbox caps, page sizes), admin transfer, manual application status override, and protocol version metadata. Admin-only — non-admin callers get `programMessage: NotAdmin`. The mainnet admin identity is held by the network team and is not the same as `admin operator` or any operator account; **do not** call `Admin/SetApplicationStatus` to promote your own application past `Building`. Use `Registry/SubmitApplication` (owner self-call) for the `Building → Submitted` step; Gear Foundation judges handle `Submitted → Live` or rejection back to `Building`.
...
### `ReviewService`
Public Gear Foundation review flow. Full review history is event/indexer-backed; on-chain state only stores judge membership, revision guards, request state, and the latest summary.
- `RequestReview(program_id, reason)` — owner-only while `Building`; asks for public feedback without changing status.
- `PostJudgeComment(program_id, expected_revision, body)` — active judge public comment for `Building` or `Submitted`.
- `OwnerReply(program_id, expected_revision, body)` — owner public reply for `Building` or `Submitted`.
- `DecideAccepted(program_id, expected_revision, reason, criteria)` — active judge accepts a `Submitted` revision to `Live`.
- `DecideRejected(program_id, expected_revision, reason, criteria)` — active judge rejects a `Submitted` revision back to `Building`; the next pending revision increments.
