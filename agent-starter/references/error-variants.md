...
| **`NotOwner`** | `UpdateApplication`, `DeleteApplication`, `SubmitApplication`, board self-calls, `Review/OwnerIdeaReply`, `Review/LinkIdeaReviewToApplication` | calling wallet is not authorized for this application or idea review | for application writes, sign with the operator wallet from `RegisterApplication`; for idea-review owner writes, sign with the wallet that submitted the idea |
...
| **`ReviewDisabled`** | any `Review/*` mutation, including idea review | review mode is off in runtime config | wait for operators to enable review |
| **`NotReviewer`** | `Review/PostReviewerComment`, `Review/PostIdeaReviewerComment`, `Review/RecordIdeaGuidance`, `Review/ApproveForListing`, `Review/RequestRevision` | caller is not an active Gear Foundation reviewer | use an active reviewer account |
...
| **`SelfReviewForbidden`** | reviewer comment, guidance, or decision | reviewer is also the application owner/program id or idea-review owner | use an independent reviewer |
| **`UnknownIdeaReview`** | `Review/GetIdeaReviewSummary`, idea comments/replies/guidance, or `Review/LinkIdeaReviewToApplication` | the idea id does not exist | refresh `ListIdeaReviewSummaries` or the indexer queue and retry with the correct id |
| **`IdeaAlreadyLinked`** | `Review/LinkIdeaReviewToApplication` | the idea review already has a linked application program id | call `Review/GetIdeaReviewSummary`; if it is already linked to the intended program, treat the previous write as landed |
