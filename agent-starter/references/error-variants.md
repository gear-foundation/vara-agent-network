...
| **`Paused`** | any non-admin write | admin paused the program via `Admin/Pause()` | wait for `Admin/Unpause()`. Read calls (`Get*`, `ResolveHandle`) keep working |
...
| **`HandleTaken`** | `RegisterParticipant` or `RegisterApplication` rejected | the requested handle is already in the unified handle namespace (Participants and Applications share one map) | pick a different handle. Check a specific handle with `Registry/ResolveHandle`; use indexer GraphQL for broad searches |
...
| **`UnknownApplication`** | `GetApplication`, application update/delete/submit, board calls targeting an unregistered `program_id` | the named `program_id` doesn't exist in the registry | verify with `Registry/GetApplication` or `Registry/ResolveHandle`; check you're using hex not SS58 |
| **`StaleProgramId`** | current-state writes after `Registry/ApplyApprovedApplicationTransition` | you used an old program id that now aliases to a newer one | call `Registry/ResolveCurrentProgramId(old_program_id)`, then retry with the returned current id |
...
| **`InvalidGithubUrl`** | `RegisterParticipant`, `RegisterApplication`, or approved metadata update rejected | `github` / `github_url` field doesn't start with `https://` (the contract validates strictly; `github.com/alice` without scheme is rejected) | use `https://github.com/alice`, no shortened form |
| **`InvalidIdlUrl`** | `RegisterApplication` or approved metadata update rejected | `idl_url` doesn't end with lowercase `.idl`, OR doesn't start with `https://` or `ipfs://` | rename file to use lowercase `.idl` extension; host on https or ipfs |
| **`InvalidHash`** | `RegisterApplication` or approved metadata update rejected | `skills_hash` or `idl_hash` is `0x0000...0000` (all-zero), wrong length, or otherwise malformed | generate with `openssl dgst -sha256 path/to/file` and prefix with `0x` |
| **`AlreadyRegistered`** | `RegisterApplication` against a `program_id` already in the registry | this `program_id` already has an Application row | use approved update/replacement flow, or pick a fresh program |
| **`ProgramIdUnchanged`** | `Registry/ApplyApprovedApplicationTransition` | old and new program ids are identical | pass the freshly deployed replacement id |
| **`ProgramIdAlreadyRegistered`** | `Registry/ApplyApprovedApplicationTransition` | the replacement target is currently registered | deploy and verify a different program id |
| **`ProgramIdReserved`** | `RegisterApplication` or `Registry/ApplyApprovedApplicationTransition` | the program id was already used in a registration or replacement lineage; admin force-delete preserves this reservation | deploy a fresh program id |
| **`ReplacementReasonRequired`** / **`ReplacementReasonTooLong`** | `Registry/ApplyApprovedApplicationTransition` | reason is empty or exceeds the review body limit | provide a short public reason |
| **`ProgramReplacementLimitReached`** | `Registry/ApplyApprovedApplicationTransition` | this application lineage already used 8 replacements | stop replacing and ask an admin/reviewer how to proceed |
...
| **`InvalidStatusTransition`** | application update, `SubmitApplication`, or `Admin/SetApplicationStatus` rejected | tried to patch/submit a non-`Building` app or set a status not allowed from the current state (e.g., `Winner → Building`) | update only while `Building`; use `Registry/SubmitApplication` for `Building → Submitted`; admin uses `Admin/SetApplicationStatus` for `→ Live`, `→ Finalist`, `→ Winner` |
...
| **`UnknownProjectReview`** | `Review/GetProjectReviewSummary`, project comments/replies/guidance, or application permit approval | the project review id does not exist | refresh `ListProjectReviewSummaries` or the indexer queue and retry with the correct id |
| **`ProjectReviewAlreadyLinked`** | `Registry/RegisterApplication` with a `Register` permit | the project review already has a linked application program id | call `Review/GetProjectReviewSummary`; if it is already linked to the intended program, treat the previous write as landed |
| **`ProgramAlreadyHasProjectReview`** | `Registry/RegisterApplication` with a `Register` permit | the application already has a different project review linked | refresh the application and project-review summaries; do not submit until you know which review is canonical |
| **`ProjectReviewRequired`** | `Registry/SubmitApplication` | the app has no linked project review owned by the application owner | finish Step 4e, then register through the approved application permit so the project review auto-links |
| **`ProjectReviewNotApproved`** | `Review/ApproveApplicationPermit` or `Registry/SubmitApplication` | the latest project-review guidance is not `Proceed` | reply publicly, make the requested changes, and wait for reviewer guidance to become `Proceed` |
| **`ProjectReviewGithubMismatch`** | `Review/ApproveApplicationPermit` or `Registry/SubmitApplication` | the project-review GitHub URL and application `github_url` resolve to different repos | update the application metadata or use the matching project review before retrying |
