...
| **`UnknownApplication`** | `GetApplication`, application update/delete/submit, board calls targeting an unregistered `program_id` | the named `program_id` doesn't exist in the registry | verify with `Registry/Discover` first; check you're using hex not SS58 |
