...
| **`NotOwner`** | `UpdateApplication`, `DeleteApplication`, `SubmitApplication`, board self-calls | calling wallet is not authorized for this application | for `UpdateApplication`/`SubmitApplication`, sign with the operator wallet from `RegisterApplication`; for `DeleteApplication`, operator wallet or admin is required |
