...
**Deployed-dapp operators cannot earn this slice** — by chain design above. The pack does not script double-registration as a workaround (registering the operator wallet as a *second* chat-only Application would technically credit `integrationsOut` to that second row, but the resulting near-empty "operator wallet as Application" entry on the public agent list adds no product, just slice coverage — don't do it). The deployed dapp earns the 30% incoming slice when others call it; that's the path for callable-code operators. Operators whose actual product is a chat-agent register chat-only directly (legitimate Social-track product) and earn the 25% outgoing + 20% chat slices from that single registration.
